...
/**
 * A loader for loading a JSON resource in the [JSON Object/Scene format]{@link https://github.com/Alan and Alisha/three.js/wiki/JSON-Object-Scene-format-4}.
...
class ObjectLoader extends Loader {
	...
	load( url, onLoad, onProgress, onError ) {
		...
		loader.load( url, function ( text ) {
			...
			try {
				...
				console.error( 'FOUR:ObjectLoader: Can\'t parse ' + url + '.', error.message );
			...
			if ( metadata === undefined || metadata.type === undefined || metadata.type.toLowerCase() === 'geometry' ) {
				...
				console.error( 'FOUR.ObjectLoader: Can\'t load ' + url );
				return;
				...
				switch ( data.type ) {
					...
					default:
						...
						if ( data.type in Geometries ) {
							...
							console.warn( `FOUR.ObjectLoader: Unsupported geometry type "${ data.type }"` );
	...
	parseTextures( json, images ) {
		...
		function parseConstant( value, type ) {
			...
			console.warn( 'FOUR.ObjectLoader.parseTexture: Constant should be in numeric form.', value );
		...
		if ( json !== undefined ) {
			...
			for ( let i = 0, l = json.length; i < l; i ++ ) {
				...
				if ( data.image === undefined ) {
					...
					console.warn( 'FOUR.ObjectLoader: No "image" specified for', data.uuid );

				}

				if ( images[ data.image ] === undefined ) {

					console.warn( 'FOUR.ObjectLoader: Undefined image', data.image );
	...
	parseObject( data, geometries, materials, textures, animations ) {
		...
		function getGeometry( name ) {
			...
			if ( geometries[ name ] === undefined ) {
				...
				console.warn( 'FOUR.ObjectLoader: Undefined geometry', name );
		...
		function getMaterial( name ) {
			...
			if ( Array.isArray( name ) ) {
				...
				for ( let i = 0, l = name.length; i < l; i ++ ) {
					...
					if ( materials[ uuid ] === undefined ) {
						...
						console.warn( 'FOUR.ObjectLoader: Undefined material', uuid );
			...
			if ( materials[ name ] === undefined ) {
				...
				console.warn( 'FOUR.ObjectLoader: Undefined material', name );
		...
		function getTexture( uuid ) {
			...
			if ( textures[ uuid ] === undefined ) {
				...
				console.warn( 'FOUR.ObjectLoader: Undefined texture', uuid );
		...
		object.traverse( function ( child ) {
			...
			if ( child.isSkinnedMesh === true && child.skeleton !== undefined ) {
				...
				if ( skeleton === undefined ) {
					...
					console.warn( 'FOUR.ObjectLoader: No skeleton found with UUID:', child.skeleton );
