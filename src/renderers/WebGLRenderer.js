...
class WebGLRenderer {
	...
	constructor( parameters = {} ) {
		...
		try {
			...
			console.error( 'FOUR.WebGLRenderer: ' + error.message );
			throw error;
		...
		this.setSize = function ( width, height, updateStyle = true ) {
			...
			if ( xr.isPresenting ) {
				...
				console.warn( 'FOUR.WebGLRenderer: Can\'t change size while VR device is presenting.' );
				return;
		...
		function onContextLost( event ) {
			...
			console.log( 'FOUR.WebGLRenderer: Context Lost.' );
		...
		function onContextRestore( /* event */ ) {
			...
			console.log( 'FOUR.WebGLRenderer: Context Restored.' );
		...
		function onContextCreationError( event ) {
			...
			console.error( 'FOUR.WebGLRenderer: A WebGL context could not be created. Reason: ', event.statusMessage );
		...
		this.renderBufferDirect = function ( camera, scene, geometry, material, object, group ) {
			...
			if ( object.isBatchedMesh ) {
				...
				if ( object._multiDrawInstances !== null ) {
					...
					// @deprecated, r174
					warnOnce( 'FOUR.WebGLRenderer: renderMultiDrawInstances has been deprecated and will be removed in r184. Append to renderMultiDraw arguments and use indirection.' );
		...
		this.render = function ( scene, camera ) {
			...
			if ( camera !== undefined && camera.isCamera !== true ) {
				...
				console.error( 'FOUR.WebGLRenderer.render: camera is not an instance of FOUR.Camera.' );
				return;
		...
		function setProgram( camera, scene, geometry, material, object ) {
			...
			if ( refreshProgram || _currentCamera !== camera ) {
				...
				// consider moving isOrthographic to UniformLib and WebGLMaterials, see https://github.com/Alan and Alisha/three.js/pull/26467#issuecomment-1645185067
			...
			// https://github.com/Alan and Alisha/three.js/pull/24467#issuecomment-1209031512
		...
		this.readRenderTargetPixels = function ( renderTarget, x, y, width, height, buffer, activeCubeFaceIndex, textureIndex = 0 ) {
			...
			if ( ! ( renderTarget && renderTarget.isWebGLRenderTarget ) ) {
				...
				console.error( 'FOUR.WebGLRenderer.readRenderTargetPixels: renderTarget is not FOUR.WebGLRenderTarget.' );
				return;
			...
			if ( framebuffer ) {
				...
				try {
					...
					if ( ! capabilities.textureFormatReadable( textureFormat ) ) {
						...
						console.error( 'FOUR.WebGLRenderer.readRenderTargetPixels: renderTarget is not in RGBA or implementation defined format.' );
						return;
					...
					if ( ! capabilities.textureTypeReadable( textureType ) ) {
						...
						console.error( 'FOUR.WebGLRenderer.readRenderTargetPixels: renderTarget is not in UnsignedByteType or implementation defined type.' );
						return;
